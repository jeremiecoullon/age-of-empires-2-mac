...
- **Drop-off buildings must fail placement if no resource found**: When `build_near_resource()` is called for mills, lumber camps, or mining camps, if `_find_nearest_resource_position()` returns `Vector2.ZERO` (no resource of that type exists), return `Vector2.ZERO` to fail placement. Don't fall back to the TC position - a mill/lumber_camp/mining_camp next to TC is useless since TC already accepts those resource types. The rule will retry on future ticks when resources become available.
